...
/**
 * Territory visibility scope — DemandCreator::visibleTo($viewer).
 */
...
test('a sales_rep sees demand creators only in their own active position territory', function (): void {
    ...
    $visible = DemandCreator::visibleTo($rep)->pluck('id')->all();
...
test('a sales_rep with no active position sees nothing', function (): void {
    DemandCreator::factory()->create(['territory_id' => $this->terrA->id]);
...
test('a regional_head sees every demand creator in their region and none outside it', function (): void {
    ...
    $inRegion = DemandCreator::factory()->create(['territory_id' => $this->terrA->id]);
    $outOfRegion = DemandCreator::factory()->create(['territory_id' => $this->terrB->id]);

    $visible = DemandCreator::visibleTo($head)->pluck('id')->all();
...
test('a regional_head with no region sees nothing (never falls through to all)', function (): void {
    DemandCreator::factory()->create(['territory_id' => $this->terrA->id]);
    DemandCreator::factory()->create(['territory_id' => $this->terrB->id]);

    $head = User::factory()->withRole('regional_head')->create(); // region_id null

    expect(DemandCreator::visibleTo($head)->count())->toBe(0);
});

test('national roles see every demand creator', function (string $role): void {
    DemandCreator::factory()->create(['territory_id' => $this->terrA->id]);
    DemandCreator::factory()->create(['territory_id' => $this->terrB->id]);

    $user = User::factory()->withRole($role)->create();

    expect(DemandCreator::visibleTo($user)->count())->toBe(2);
})->with([
    'superuser' => ['superuser'],
